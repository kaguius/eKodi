<?php
	...
	if($adminstatus == 3){
		...
		$page_title = "Landlord Statement Report";
		include_once('includes/header.php');
		if (!empty($_GET)){	
		...
		if ($report_start_date != "" || $report_end_date != "" || $property_name != ""){
			...
			$sql = mysql_query("select id, property_name, property_owner, property_type, location, propety_contact, commission, taxes, water_cost from property_details where id = '$property_id'");
					...
					<div id="content">
						<div class="post">
							...
							<h2 align='right'>Landlord Statement</h2>
								...
								<strong>Statement Month: </strong> <?php echo $actual_rent_month .", ".$rent_year ?><br />
								<strong>Property Type: </strong><?php echo $property_type ?><br /><br />
		...
		<div id="page">
			<div id="content">
				<div class="post">
					<h2><?php echo $page_title ?> | e-kodi</h2>
						<form id="frmDeptCategories" name="frmDeptCategories" method="GET" action="statement.php">
	...
	<?php
			...
			if (!empty($_POST)) {
				...
				$userid= $_POST['userid'];
				$property_manager_id= $_POST['property_manager_id'];
				$payment_type= $_POST['payment_type'];
				$net_revenue= $_POST['net_revenue'];
				$trans_id= $_POST['trans_id'];

				$sql3="
					INSERT INTO property_owner_remittances (payment_type, property_id, landlord_name, amount, remmitance_month, remmitance_year, manager_id, UID, transactiontime, trans_id)
					VALUES('$payment_type', '$property_id', '$property_owner', '$net_revenue', '$rem_month', '$rem_year', '$property_manager_id', '$userid', '$transactiontime', '$trans_id')";

				//echo $sql3;
				$result = mysql_query($sql3);
				?>
					<script type="text/javascript">
						document.location = "statement.php";
					</script>
				<?php
			}
		}
